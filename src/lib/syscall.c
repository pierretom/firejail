...
void is_groups_exist(const char *groups_list)
{
	int i;
	int elems = sizeof(sysgroups) / sizeof(sysgroups[0]);
	int no_match = 0;
	...
	if (new_str[0] != '\0') {
		...
		fprintf(stderr, "See --debug-groups for valid groups\n");
		free(copy);
		exit(1);
	}
...
void groups_print_syscalls(const char *groups_list) {
	if (groups_list == NULL || *groups_list == '\0') {
		fprintf(stderr, "Value for --debug-groups can not be empty\n");
		exit(1);
	}

	is_groups_exist(groups_list);
